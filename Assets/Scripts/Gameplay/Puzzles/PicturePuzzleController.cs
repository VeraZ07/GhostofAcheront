...
namespace GOA
{
    public class PicturePuzzleController : PuzzleController
    {
        ...
        public override void Spawned()
        {
            ...
            Transform triggersRoot = new List<Transform>(pictureObject.GetComponentsInChildren<Transform>()).Find(t => t.gameObject.name.ToLower().Equals("triggers"));
            for (int i = 0; i < triggersRoot.childCount; i++)
            {
                PieceInteractor interactor = triggersRoot.GetChild(i).gameObject.GetComponent<PieceInteractor>();
